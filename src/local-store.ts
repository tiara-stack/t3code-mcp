        ...
        const captureId = yield* crypto.randomUUIDv4.pipe(
          Effect.mapError(
            () =>
              new LocalStoreError({
                kind: "storage",
                message: `Could not create a ${input.captureKind} capture.`,
              }),
          ),
        );
        const now = yield* Clock.currentTimeMillis;
